...
public class Transcoder implements Runnable
{
  ...
  private  Boolean sOnce = false;
  private  PipedOutputStream pos = null;
  
  
  private static SourceDataLine mLine;
  private  int countz = 0;
  private Boolean bPipeClosed = true;
  ...
  {
    this(aInputStream, aOutputStream, aOutputInfo, null, null, null);
  }
  ...
  private static SourceDataLine getDataline(AudioFormat format)
  {
	  
	  try
	  {
		  return null;
		//  pos = new PipedOutputStream(mPis);
	  }
	  catch(Exception e)
	  {
		  e.printStackTrace();
  ...
  private  void openJavaSound(IStreamCoder aAudioCoder)
  {
	  ...
	  sOnce=true;
	  int wtff=(int)IAudioSamples.findSampleBitDepth(aAudioCoder.getSampleFormat());
	  int wtff2=aAudioCoder.getSampleRate();
	  int wtff3=aAudioCoder.getChannels();
  ...
  public void closer()
  {
	  ...
	  {
		  log.warn("Exception closing transcoder");
		  e.printStackTrace();
	  }
  }
  ...
  private void playJavaSound(IAudioSamples aSamples) {
		...
		if ((!bPipeClosed) && (pos != null)) {
			// mLine.write(rawBytes, 0, aSamples.getSize());
			try {
				countz++;
				log.debug("countz is " + countz + " size is" + aSamples.getSize());
				pos.write(rawBytes, 0, aSamples.getSize());
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
  ...
  public void setOutputStream(PipedOutputStream apos)
  {
	  bPipeClosed = false;
	   pos=apos;
  }
  ...
  private void writePacket(IPacket oPacket)
  {
		...
		/*
		 * We allocate a set of samples with the same number of channels as the coder tells us is in this
		 * buffer.
		 * 
		 * We also pass in a buffer size (1024 in our example), although Xuggler will probably allocate more
		 * space than just the 1024 (it's not important why).
		 */
		IAudioSamples samples = IAudioSamples.make(1024, mOutAudioCoder.getChannels());
		openJavaSound(mOutAudioCoder);
		/*
		 * A packet can actually contain multiple sets of samples (or frames of samples in audio-decoding
		 * speak). So, we may need to call decode audio multiple times at different offsets in the packet's
		 ...
		 */
		while (offset < oPacket.getSize()) {
			int bytesDecoded = mOutAudioCoder.decodeAudio(samples, oPacket, offset);
			if (bytesDecoded < 0)
				throw new RuntimeException("GGGGGGGGGGGGGGGGGG ");
			offset += bytesDecoded;
			/*
			 * Some decoder will consume data in a packet, but will not be able to construct a full set of
			 * samples yet. Therefore you should always check if you got a complete set of samples from the
			 * decoder
			 */
